-- بداية التعديلات على قاعدة البيانات في Supabase
-- هذا الكود يجب تشغيله مرة واحدة في محرر SQL لتطبيق التعديلات اللازمة

-- أولاً: إزالة الوظائف الحالية لتجنب أخطاء تغيير أسماء المعاملات
DROP FUNCTION IF EXISTS get_project_settings();
DROP FUNCTION IF EXISTS update_project_settings(text, text, integer, jsonb);
DROP FUNCTION IF EXISTS get_fixed_costs();
DROP FUNCTION IF EXISTS upsert_fixed_cost(uuid, text, numeric, text);
DROP FUNCTION IF EXISTS upsert_fixed_cost(uuid, uuid, text, numeric, text);
DROP FUNCTION IF EXISTS delete_fixed_cost(uuid);

-- 1. تحديث وظيفة get_project_settings لإضافة دعم معرف المستخدم المرسل صراحةً
...
-- 2. تحديث وظيفة update_project_settings لإضافة دعم معرف المستخدم المرسل صراحةً
...
BEGIN
  ...
  ELSE
    ...
    SET 
      ...
      target_category = COALESCE(p_target_category, target_category),
      target_profit = COALESCE(p_target_profit, target_profit),
      updated_at = NOW()
...
-- 3. تحديث وظيفة get_fixed_costs لإضافة دعم معرف المستخدم المرسل صراحةً
...
-- 4. تحديث وظيفة upsert_fixed_cost لإضافة دعم معرف المستخدم المرسل صراحةً
...
-- 5. تحديث وظيفة delete_fixed_cost لإضافة دعم معرف المستخدم المرسل صراحةً
...
-- 6. تشغيل تحديث على جدول project_settings لتأكد من العلاقات والقيود
DO
$$
BEGIN
  -- إصلاح أي إعدادات معلقة بدون user_id صحيح (إذا وجدت)
  -- هذا سيحذف الإعدادات التي ليس لها مستخدم مناظر
  DELETE FROM project_settings
  WHERE user_id NOT IN (SELECT id FROM auth.users);

  RAISE NOTICE 'تم فحص وإصلاح جداول project_settings';
END
$$;

-- 7. إنشاء وظيفة للتحقق من حالة المستخدم - مفيدة للتشخيص
...
SELECT 'تم تنفيذ كافة التعديلات بنجاح' AS result;