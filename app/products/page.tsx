    ...
    return (
      <AppShell>
        <div className="space-y-6">
          {/* بطاقة الملخص الرئيسية - نسخة مبسطة */}
              ...
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {mostProfitableProducts.map((product, index) => {
                    ...
                    return (
                      <div key={product.id} className="border border-gray-100 rounded-lg p-4 bg-white/80 hover:border-indigo-100 hover:shadow-sm transition-all duration-200">
                        <div className="flex justify-between items-start gap-2">
                          <h3 className="font-medium text-gray-800 truncate flex-1" title={product.name}>{product.name}</h3>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <Link href={`/pricing/calculator?product_id=${product.id}`}>
                            <Button variant="outline" size="sm" className="w-full border-green-200 hover:bg-green-50 text-green-700 text-xs h-8">
...
function calculateAverageProfit(products: Product[]): number {
  const pricedProducts = products.filter(
    (p) => p.has_pricing && p.price && p.cost && p.cost > 0
  );
