...
export default function ProductsList({ initialProducts }: ProductsListProps) {
  ...
  const [sortOption, setSortOption] = useState<string>('recent') // 'recent', 'name', 'profit'
  ...
  const refreshProducts = async () => {
    try {
      ...
      // إعادة تعيين المنتجات المحددة عند تحديث القائمة
      setSelectedProducts([])
    } catch (error) {
  ...
  const applyFilters = useCallback((productsList: Product[], term: string, filterType: string, sort: string) => {
    ...
    switch (sort) {
      ...
      case 'recent':
      default:
        // المنتجات تكون مرتبة بالفعل حسب تاريخ الإنشاء من الاستعلام
        break;
    }
    ...
    <section className="space-y-6">
      ...
      <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div className={`${isRTL ? 'order-2 sm:order-1' : ''} w-full sm:w-auto`}>
              ...
              <p className="text-muted-foreground text-sm mt-1">
                {filteredProducts.length} منتج • {products.filter(p => p.has_pricing).length} مسعّر • {products.filter(p => !p.has_pricing).length} بانتظار التسعير
              </p>
            </div>
            <div className={`flex ${isRTL ? 'justify-end order-1 sm:order-2' : 'justify-start'} gap-2 w-full sm:w-auto`}>
              <Button 
          ...
          {/* شريط البحث والفلترة - معدل للجوال */}
          <div className={`flex flex-col ${isMobile ? '' : 'sm:flex-row'} gap-3 items-stretch sm:items-center`}>
            ...
            <div className={`flex ${isMobile && isRTL ? 'flex-row-reverse justify-end' : 'justify-start'} gap-2 overflow-x-auto pb-2 sm:pb-0`}>
              <Button 
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" size="sm" className={`border-gray-200 gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <AnimatePresence>
              {filteredProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    <div className={`absolute top-2 ${isRTL ? 'left-2' : 'right-2'} z-10 ${isProductSelected(product.id) ? 'visible' : 'invisible group-hover:visible'}`}>
                      <input
                        type="checkbox"
                    ...
                    <Card className={`h-full border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden ${isProductSelected(product.id) ? 'border-2 border-indigo-500' : ''}`}>
                      <div className="p-4 border-b border-gray-100">
                        {/* عرض المنتج - تعديل التنسيق لضمان العرض من اليمين إلى اليسار */}
            ...
            <table className="min-w-full bg-white divide-y divide-gray-200 rounded-lg border border-gray-100">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProducts.map((product) => {
                  const profitMargin = calculateProfitMargin(product);
                  return (
                    <tr key={product.id} className={`hover:bg-gray-50 ${isProductSelected(product.id) ? 'bg-indigo-50' : ''}`}>
                      <td className="p-4">
                        <Input 
                          type="checkbox" 
                          checked={isProductSelected(product.id)}
                      ...
                      <td className="p-4">
                        <div className="flex items-center">
                          <div className="h-10 w-10 flex-shrink-0 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-center mr-3">
                            {product.image_url ? (
                              <Image src={product.image_url} alt={product.name} width={30} height={30} className="object-cover" />
                            ) : (
          ...
          <Card className="w-full max-w-md mx-4">
            ...
            <CardContent>
              <Alert className="bg-red-50 text-red-800 border border-red-200">
                ...
                <AlertDescription>
                  سيتم حذف هذه المنتجات بشكل نهائي من قاعدة البيانات.
                </AlertDescription>
              </Alert>
            </CardContent>
